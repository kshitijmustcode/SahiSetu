...
Do not use unsupported rates, savings, targets, or outcome projections in the product or pitch. The homepage contains only two source-checked references: MoRTH's 2023–24 Annual Report says the National Registry had **20.26+ crore driving-licence records as of 31 March 2024** (registry size, not annual transactions or SahiSetu's addressable market), linked through the official [MoRTH website](https://morth.gov.in/#/); and [India Code: Central Act No. 59 of 1988](https://www.indiacode.nic.in/handle/123456789/1802), whose section 181 states a ₹5,000 fine for driving in contravention of sections 3 or 4 (or imprisonment up to three months, or both). Do not simplify this to “an expired licence automatically attracts a fine,” and do not claim contact confirmation avoids a penalty.

The only product benefits presented are concrete and demonstrable: Aarohi can spot expiry and preparation gaps early; Rohan can catch address-entry differences before proceeding; Neha can verify an unresolved payment before considering another payment. Actual evidence must come from the small usability study in `USER_TESTING.md`, including failures and limitations.
...
- `app/components/site-chrome.tsx` owns the shared SahiSetu navigation and footer. Keep the document check’s compact progress toolbar as a deliberate exception for its focused three-step flow.
...
- Shared explainable timeline: `app/components/explainable-audit-timeline.tsx`. It is used by the address-result audit, Application Rescue, and the shared staff triage view. Its fixed order is source record → extracted fields → citizen confirmation → readiness result → safe next action.
- Shared demo journey state: `app/lib/demo-journey-state.ts`. It stores only browser-local flags—Aarohi contact readiness, medical pre-check, and readiness packet; Rohan readiness packet; and Neha support summary—plus the address-review snapshot (proof wording, final citizen text, minor-difference flag, clarification-note signature). This lets `/triage` and `/handoff` reflect completed citizen steps. It must never be described as a real citizen/RTO record.
...
Additional recent commits include `d7a84ef` (flagship readiness and user-testing guide) and `0703fbd` (benchmark context, subsequently tightened to source-checked national context and concrete benefits). Confirm `git status` before beginning new work rather than assuming the worktree is uncommitted.
