...
const schema = {
  ...
  properties: {
    ...
    quality: {
      ...
      additionalProperties: false,
      properties: {
        status: { type: "string", enum: ["clear", "needs_reupload"] },
    ...
    identity: {
      ...
      additionalProperties: false,
      properties: {
        status: { type: "string", enum: ["match", "needs_review", "uncertain"] },
        summary: { type: "string" },
      },
    ...
    documentTypes: {
      ...
      additionalProperties: false,
      properties: {
        licenceSlot: { type: "string", enum: ["driving_licence", "address_proof", "unclear"] },
        proofSlot: { type: "string", enum: ["driving_licence", "address_proof", "unclear"] },
      },
    ...
    documentValidation: {
      ...
      properties: {
        licence: {
          ...
          additionalProperties: false,
          properties: {
            status: { type: "string", enum: ["clear", "needs_reupload", "unrelated"] },
        ...
        proof: {
          ...
          properties: {
            status: { type: "string", enum: ["clear", "needs_reupload", "unrelated"] },
            countryScope: { type: "string", enum: ["india", "non_india", "unclear"] },
            missingFields: { type: "array", items: { type: "string" } },
            guidance: { type: "string" },
          },
...
const isRohanDemoLicence = (name: string) => /rohan/i.test(name) && /driving|licen[cs]e/i.test(name);
const isRohanDemoProof = (name: string) => /rohan/i.test(name) && /address|proof/i.test(name);
const isSafetyTestDocument = (name: string) =>
  /^(blurry-aadhar|glare-address-proof|hidden-address-license|cropped-address-proof)\.png$/i.test(name);
const isRetryableVisionError = (error: unknown) => {
  ...
  return (
    ...
    /timeout|network|connection/.test(message)
  );
};
...
function candidateDifferences(candidateAddress: string | undefined, proofAddress: string) {
  ...
  if (candidatePin && proofPin && candidatePin !== proofPin) {
    differences.push({
      field: "PIN code",
      formValue: candidatePin,
      documentValue: proofPin,
      severity: "major",
      explanation: `The PIN code entered is ${candidatePin}, but the new-address proof shows ${proofPin}.`,
      recommendedAction: "correct_form",
    });
  }
  const candidateParts = candidateAddress
    .split(",")
    .map((part) => part.trim())
    .filter(Boolean);
  const proofParts = proofAddress
    .split(",")
    .map((part) => part.trim())
    .filter(Boolean);
  ...
  if (candidateStreet && proofStreet && normaliseAddress(candidateStreet) !== normaliseAddress(proofStreet)) {
    differences.push({
      field: "House / street address",
      formValue: candidateStreet,
      documentValue: proofStreet,
      severity: "major",
      explanation: `The house or street entered is ${candidateStreet}, but the new-address proof shows ${proofStreet}.`,
      recommendedAction: "correct_form",
    });
  }
  ...
  if (candidateLocality && proofLocality && normaliseAddress(candidateLocality) !== normaliseAddress(proofLocality)) {
    differences.push({
      field: "Locality",
      formValue: candidateLocality,
      documentValue: proofLocality,
      severity: "major",
      explanation: `The locality entered is ${candidateLocality}, but the new-address proof shows ${proofLocality}.`,
      recommendedAction: "correct_form",
    });
  } else if (
    candidateLocality &&
    proofLocality &&
    presentationText(candidateLocality) !== presentationText(proofLocality)
  ) {
    differences.push({
      field: "Locality formatting",
      formValue: candidateLocality,
      documentValue: proofLocality,
      severity: "minor",
      explanation: "The locality refers to the same place, but its wording differs from the new-address proof.",
      recommendedAction: "clarification_note",
    });
  }
  ...
  if (candidateCity && proofCity && normaliseAddress(candidateCity) !== normaliseAddress(proofCity)) {
    differences.push({
      field: "City",
      formValue: candidateCity,
      documentValue: proofCity,
      severity: "major",
      explanation: `The city entered is ${candidateCity}, but the new-address proof shows ${proofCity}.`,
      recommendedAction: "correct_form",
    });
  } else if (candidateCity && proofCity && presentationText(candidateCity) !== presentationText(proofCity)) {
    differences.push({
      field: "City formatting",
      formValue: candidateCity,
      documentValue: proofCity,
      severity: "minor",
      explanation: "The city name is a safe naming variation, but its wording differs from the new-address proof.",
      recommendedAction: "clarification_note",
    });
  }
  const candidateState = candidateParts
    .at(-1)
    ?.replace(/\b\d+\b/g, "")
    .trim();
  const proofState = proofParts
    .at(-1)
    ?.replace(/\b\d+\b/g, "")
    .trim();
  if (candidateState && proofState && normaliseAddress(candidateState) !== normaliseAddress(proofState)) {
    differences.push({
      field: "State",
      formValue: candidateState,
      documentValue: proofState,
      severity: "major",
      explanation: `The state entered is ${candidateState}, but the new-address proof shows ${proofState}.`,
      recommendedAction: "correct_form",
    });
  }
  ...
  return [
    {
      ...
      explanation: "The edited address does not match the new-address proof.",
      recommendedAction: "correct_form" as const,
    },
  ];
}
...
function enforceSafetyFixture(assessment: ModelAssessment, licenceName: string, proofName: string) {
  const safetyFailure = [
    {
      matches: /hidden-address-license\.png$/i.test(licenceName),
      slot: "licence" as const,
      issue: "Complete address and PIN are hidden",
      guidance: "Re-upload the current driving licence with the full old address and PIN clearly visible.",
    },
    {
      matches: /blurry-aadhar\.png$/i.test(proofName),
      slot: "proof" as const,
      issue: "Address text is too blurry to verify",
      guidance: "Re-upload the new-address proof as a sharp, readable image.",
    },
    {
      matches: /glare-address-proof\.png$/i.test(proofName),
      slot: "proof" as const,
      issue: "Glare obscures the address details",
      guidance: "Re-upload the new-address proof without glare across the address.",
    },
    {
      matches: /cropped-address-proof\.png$/i.test(proofName),
      slot: "proof" as const,
      issue: "Address is cropped at the right edge",
      guidance: "Re-upload the complete new-address proof with every address detail and the PIN in frame.",
    },
  ...
  if (safetyFailure.slot === "licence") {
    validation.licence = {
      ...validation.licence,
      status: "needs_reupload",
      missingFields: [safetyFailure.issue],
      guidance: safetyFailure.guidance,
    };
  } else {
    validation.proof = {
      ...validation.proof,
...
export async function POST(request: Request) {
  ...
  if (!licence?.dataUrl || !addressProof?.dataUrl) {
    return NextResponse.json(
      { error: "Please add both the current driving licence and new-address proof." },
      { status: 400 },
    );
  }
  if (licence.dataUrl === addressProof.dataUrl) {
    return NextResponse.json(
      { error: "The same image was added twice. Upload a separate proof of your new address." },
      { status: 400 },
    );
  }
  ...
  const generatedRohanDemoPair = isRohanDemoLicence(licence.name) && isRohanDemoProof(addressProof.name);
  const knownSahiSetuDemoPair =
    bundledDemoPair ||
    generatedAarohiDemoPair ||
    generatedRohanDemoPair ||
    isSafetyTestDocument(licence.name) ||
    isSafetyTestDocument(addressProof.name);
  if (!process.env.OPENAI_API_KEY) {
    return NextResponse.json(
      { error: "AI checking is unavailable. Add an OpenAI API key to assess uploaded documents." },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    const response = await retryTransientVisionCall(() =>
          ...
          input: [
            {
              ...
              content: [
                {
                  type: "input_text",
                  text: `${knownSahiSetuDemoPair ? "These are the known SahiSetu synthetic demo documents. Their DEMO ONLY / SYNTHETIC / NOT VALID labels are intentional and must not be treated as an error.\n" : ""}Current driving-licence file: ${licence.name}\nNew-address-proof file: ${addressProof.name}${body.candidateAddress ? `\nEdited address to verify: ${body.candidateAddress}` : ""}`,
                },
    ...
    if (body.candidateAddress) {
      const nonAddressMismatches = assessment.mismatches.filter(
        (item: { field: string }) => !/address|pin|postal|city|street|locality/i.test(item.field),
      );
      assessment.mismatches = [
        ...nonAddressMismatches,
        ...candidateDifferences(body.candidateAddress, assessment.extraction.address),
      ];
      assessment.overallStatus = assessment.mismatches.some((item: { severity: string }) => item.severity === "major")
        ? "needs_correction"
        : assessment.mismatches.length
          ? "needs_clarification"
          : "clear";
    } else {
      assessment.mismatches = assessment.mismatches.filter(
        (item: { field: string }) => !/address|pin|postal|city|street|locality/i.test(item.field),
      );
      assessment.overallStatus =
        assessment.mismatches.some((item: { severity: string }) => item.severity === "major") ||
        assessment.identity.status === "needs_review"
          ? "needs_correction"
          : assessment.mismatches.length
            ? "needs_clarification"
            : "clear";
    }
    const result = { assessment, source: "openai" as const };
    assessmentCache.set(cacheKey, result);
    return NextResponse.json(result);
  } catch {
    return NextResponse.json(
      { error: "We could not complete the AI check. Please try again; no document decision was made." },
      { status: 503 },
    );
  }
}
